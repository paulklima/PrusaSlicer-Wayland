...
|-----------|--------|------|------------|
| PrusaSlicer (stable) | **2.9.6** | 2026-07 | No |
| PrusaSlicer (next) | 3.0 pre-release (2026) | — | No (no PR merged) |
| This fork | 2.9.6-wayland | — | Yes (this change, `feat/mmu-temp-fix`) |
...
**Practical recommendation for this fork (2.9.6-wayland)**:
...
- **Observed: 205 °C intermediate setpoint on PLA→PETG transition**
  (2026-05-11). When switching PLA → PETG (target 255 °C), the nozzle
  target briefly drops to 205 °C before ramping up to 255 °C. Source
  unclear — needs investigation of Prusament PLA and Prusament PETG
  profiles for an intermediate temp (likely `filament_toolchange_temp`,
  `idle_temperature`, or a stale `first_layer_temperature` leaking into
  the toolchange sequence). 205 is suspiciously close to PLA's normal
  print temp; could be a residual M104 emission from the unload side of
  the parser-equivalent hybrid path now landing between the new-tool
  M104 and the M109 barrier. Reproduce with a 2-color PLA→PETG slice,
  grep generated G-code for `M104 S205` and trace which code path
  emits it.
