...
cd PrusaSlicer
git checkout wayland-migration    # or whichever branch has the changes
```
...
All changes are on the `wayland-migration` branch. None of them affect
non-Linux platforms.
...
the `WAYLAND_DISPLAY` fix from the `wayland-migration` branch
