...
Historical progress log of the original migration (branch `wayland-migration`).
The current branch layout uses per-feature branches (`feat/wayland`, …) combined
into `release/X.Y.Z`; see `doc/FORK-RELEASE-STRATEGY.md`.
...
- **wxWidgets upgraded to upstream 3.3.3** (`feat/wx3.3`), bringing the
  Wayland/EGL fixes — see `doc/wxWidgets-Wayland-Migration-Linux.md` for the
  audit checklist and `doc/FORK-RELEASE-STRATEGY.md` for how it ships.
