...
Ubuntu / Debian system packages: `git build-essential autoconf automake libtool libtool-bin cmake libglu1-mesa-dev libgtk-3-dev libdbus-1-dev libwebkit2gtk-4.1-dev texinfo libhidapi-dev`. (`automake`/`libtool`/`libtool-bin` are needed by the autotools deps GMP/MPFR — without them the deps build fails at `libtoolize`; `libhidapi-dev` is required since upstream 2.9.5 or the app configure fails with "Package 'hidapi-libusb' not found".)

On Ubuntu 26.04 the system CMake is 4.x, which removed compatibility with policy versions < 3.5 that the vendored deps still request. The `Makefile` wrapper exports `CMAKE_POLICY_VERSION_MINIMUM=3.5` (and `deps/CMakeLists.txt` appends it to `DEP_CMAKE_OPTS`) so the deps build works on both CMake 3.x and 4.x.
...
Native Wayland is **implemented and working**; it lives on the `feat/wayland`
branch. For the fork's branch model and how to cut a release, see
**`doc/FORK-RELEASE-STRATEGY.md`** (feature branches `feat/*` combined into
`release/X.Y.Z`, frozen tags `vX.Y.Z-wayland`).

The Wayland support is gated by four small changes documented in `doc/wxWidgets-Wayland-Migration-Linux.md`. Critical files:
